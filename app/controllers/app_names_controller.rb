class AppNamesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @app_name.save
        ...
        format.json { render action: 'show', status: :created, location: @app_name }
      else
  ...
  def destroy
    @app_name.destroy
    respond_to do |format|
      format.html { redirect_to app_names_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def app_name_params
      params.require(:app_name).permit(:title, :players, :directions)
    end
end
